...
export default function DemoAnimation() {
  ...
  // All animation state derived from scroll progress
  const citySelected = progress >= 0.10
  const selectedVibes: string[] = progress >= 0.32 ? ['artsy', 'chill'] : progress >= 0.20 ? ['artsy'] : []
  const btnState: BtnState = progress >= 0.50 ? 'click' : progress >= 0.42 ? 'pulse' : 'idle'
  const phase: Phase = progress >= 0.65 ? 'result' : progress >= 0.53 ? 'loading' : 'form'
  const drawProgress = phase === 'result' ? Math.min(1, (progress - 0.65) / 0.13) : 0
  const stopsCount = phase === 'result' ? Math.max(0, Math.min(4, Math.floor((progress - 0.78) / 0.05))) : 0
  const scrollOffset = phase === 'result' ? Math.max(-125, -125 * Math.min(1, (progress - 0.80) / 0.09)) : 0
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen flex items-center bg-[#0a0a0a] overflow-hidden">
        ...
        <div
          className="absolute bottom-7 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1.5 pointer-events-none transition-opacity duration-700"
          style={{ opacity: progress < 0.04 ? 1 : 0 }}
        >
          <span className="text-[9px] text-warm-gray-500 tracking-[0.22em] uppercase">scroll</span>
          <div className="w-px h-5 bg-gradient-to-b from-warm-gray-500/40 to-transparent" />
        </div>
              ...
              <div className="relative rounded-b-2xl border border-white/[0.08] overflow-hidden select-none" style={{ background: '#0a0a0a', height: 560 }}>
                ...
                <div className={`absolute inset-0 overflow-hidden transition-opacity duration-500 ${phase === 'result' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                  ...
                  >
                    ...
                    {/* Custom SVG map */}
                    <div className="px-5 pb-3 rounded-xl overflow-hidden" style={{ height: 230 }}>
                      <AnimMap drawProgress={drawProgress} stopsVisible={stopsCount} />
                    </div>
                    ...
                    <div className="px-5 pb-5 flex flex-col">
                      {STOPS.map((stop, i) => (
                        <div key={i}>
                          <div
                            ...
                            style={{
                              background: '#111',
                              opacity: i < stopsCount ? 1 : 0,
                              transform: i < stopsCount ? 'none' : 'translateY(8px)',
                          ...
                          {stop.walk && (
                            <div className="flex items-center gap-2 px-3 py-1.5" style={{ opacity: i < stopsCount ? 1 : 0, transition: 'opacity 0.4s ease-out 0.15s' }}>
                      ...
                      <div
                        ...
                        style={{ opacity: stopsCount >= 4 ? 1 : 0, transition: 'opacity 0.4s ease-out 0.5s' }}
                      >
