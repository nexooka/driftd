            ...
            return (
              <FadeIn key={i} delay={i * 60}>
                <button onClick={() => setOpen(isOpen ? null : i)} className="w-full text-left group">
                  <div className={`flex items-start justify-between gap-6 py-6 border-b transition-colors duration-200 ${isOpen ? 'border-amber-400/20' : 'border-white/[0.06] hover:border-white/[0.12]'}`}>
                    <div className="flex-1">
                      <p className={`font-medium text-base md:text-lg transition-colors duration-200 ${isOpen ? 'text-amber-400' : 'text-warm-gray-300 group-hover:text-warm-white'}`}>
                    ...
                    <div className={`shrink-0 w-5 h-5 rounded-full border flex items-center justify-center mt-0.5 transition-all duration-200 ${isOpen ? 'border-amber-400/40 bg-amber-400/10' : 'border-white/15 group-hover:border-white/30'}`}>
